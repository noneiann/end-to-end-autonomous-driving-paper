...
Physical deployment initially revealed a critical failure mode: the allocentric (full-AOI) state representation caused the network to overfit to the robot's absolute pixel position rather than learn generalizable path-following features, a manifestation of the ``Clever Hans'' effect in machine learning. This issue was resolved through an architectural pivot to an egocentric perception model, wherein the CNN received only a robot-centered crop drawn on a semantically abstracted black canvas. This information bottleneck forced the convolutional layers to learn translation-invariant relative geometric features.

The resulting egocentric model successfully navigated straight paths, 90-degree turns, U-turns, and off-center recovery scenarios, demonstrating that homography-based preprocessing combined with a strict visual information bottleneck can substantially bridge the gap between offline training loss and real-world kinematic performance.
...
This validates the core hypothesis of end-to-end Behavioral Cloning: that a neural network, given a well-constructed visual state, can implicitly learn the non-linear control mapping that a PID controller approximates explicitly. The absence of derivative-term damping did not produce the oscillatory behavior commonly associated with proportional-only controllers, suggesting that the CNN learned a temporally coherent steering policy from the expert demonstrations.

Perhaps the most significant and transferable finding of this study is the decisive role of the visual information bottleneck in bridging the sim-to-real gap inherent in Behavioral Cloning. The full-AOI allocentric model achieved low validation loss during training yet failed catastrophically in physical deployment, demonstrating a textbook manifestation of covariate shift exacerbated by misleading spatial memorization. By replacing this representation with a semantically abstracted robot-centered crop on a black canvas, the network's input distribution became invariant to the robot's absolute arena position.

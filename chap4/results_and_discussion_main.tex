...
The executed CNN control action is discretized from the network's normalized steering command using the same $\pm 2.0$ dead-band, with commands greater than $+2.0$ classified as Right, those below $-2.0$ as Left, and those in between as Straight. With this convention, the matrix diagonal corresponds to correct corrective steering. The combined confusion matrix across all three track topologies is shown in Figure~\ref{fig:4.11}.

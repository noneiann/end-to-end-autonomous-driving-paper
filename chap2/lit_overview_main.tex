% ISSUE #1 — DOI / cite-key mismatches (fix in chap2/lit_overview_biblio.bib):
% Several \cite{} keys in this file resolve as "?" at compile time, indicating that the
% \bibitem / BibTeX entry key does not exactly match the \cite{} key used in the body text.
% To fix: (1) ensure every \cite{key} has an exactly matching entry key in lit_overview_biblio.bib;
% (2) check that no DOI strings contain unescaped underscores — wrap them in braces, e.g.
%     doi = {10.1000/some\_doi} should be doi = {10.1000/some_doi} (no backslash needed in BibTeX values,
%     or use \url{} if the style requires it); (3) recompile twice (pdflatex → bibtex → pdflatex → pdflatex)
%     and confirm no "?" remains in the PDF.
